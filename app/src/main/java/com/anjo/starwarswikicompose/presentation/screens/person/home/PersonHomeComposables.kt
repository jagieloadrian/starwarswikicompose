...
@Composable
fun People(navController: NavHostController, homePersonViewModel: HomePersonViewModel = hiltViewModel(),
           refresh: Boolean) {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        if (item.isLoading) {
            ...
            LazyColumn(contentPadding = PaddingValues(all = SMALL_PADDING),
                ...
                items(items = item.people.orEmpty()) { item: GetAllPeoplesQuery.Person? ->
