...
class FeedbackCardKtTest {
    ...
    fun givenFeedbackCard_whenDisplayed_thenVerifyComponents() {
        ...
        //when and then
        val userEmail = composeTestRule.onNodeWithText("User email")

        userEmail.assertIsDisplayed()
        userEmail.assertIsEnabled()
        userEmail.assertIsNotFocused()
        userEmail.performClick()
        userEmail.assertIsFocused()

        userEmail.performTextClearance()
        userEmail.performTextInput("newText")
        ...
        val button = composeTestRule.onNodeWithText("Send & Close")
