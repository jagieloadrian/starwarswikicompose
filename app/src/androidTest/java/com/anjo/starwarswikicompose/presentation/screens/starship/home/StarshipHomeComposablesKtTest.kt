...
class StarshipHomeComposablesKtTest {
    ...
    @Test
    fun givenStarships_whenDisplayed_thenAssertOps() {
        //given
        val starships = listOf(UniversalChunk("1", "name1", "desc1"),
                UniversalChunk("2", "name2", "desc2"),
                UniversalChunk("3", "name3", "desc3"))
        val starshipsState = HomeStarshipViewModel.StarshipState(starships)
        composeTestRule.setContent {
            val navController = rememberNavController()
            HomeStarshipsContent(starshipsState, navController)
        }
    ...
    @Test
    fun givenEmptyStarshipAndLoading_whenDisplayed_thenAssertOps() {
        //given
        val starshipsState = HomeStarshipViewModel.StarshipState(listOf(), true)
        composeTestRule.setContent {
            val navController = rememberNavController()
            HomeStarshipsContent(starshipsState, navController)
        }
