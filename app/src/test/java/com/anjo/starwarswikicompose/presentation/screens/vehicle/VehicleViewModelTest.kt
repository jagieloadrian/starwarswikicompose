...
class VehicleViewModelTest {
    ...
    @Test
    fun `given vehicle when getVehicle then return vehicle and related images`(): Unit = runTest {
        //given
        ...
        val expected =
            VehicleDetailState(vehicleDto = VehicleDto(id = objectId, "Title", model = "1", vehicleClass = "mass"),
                    state = DetailObjectState.SUCCESS)

        coEvery { savedStateHandle.get<String>(DETAILS_VEHICLE_ARGUMENT_KEY) } returns objectId
        coEvery { useCases.getVehicleUseCase(objectId) } returns expected.vehicleDto
